...
### BREAKING CHANGES
- **Serialization format changed**: Build codes generated by v2.0.0 are not compatible with v1.x
- **v1.0.0-1.0.2 DEPRECATED**: Those versions had incorrect skill ordering and generated codes that didn't work correctly in-game
- Build codes created with v1.x will decode differently in v2.0.0 (and correctly match GW2's actual format)
- If you have stored build codes from v1.x, they will need to be regenerated with v2.0.0

### Fixed
- **CRITICAL**: Corrected skill order in encoder and decoder to match GW2's actual format
...
- Fixed Ranger pets encoding/decoding
...
### Migration from v1.x
- Regenerate all stored build codes using v2.0.0 encoder
- Any build codes generated with v1.0.0-1.0.2 will not work correctly (missing utility2 in-game)
- The TypeScript API remains unchanged - only the binary format changed
...
### v2.0.0
- **Serialization format changed** - build codes are not backwards compatible with v1.x
- Skill encoding/decoding order corrected to match GW2's actual format
- v1.0.0-1.0.2 are deprecated (generated non-functional codes)
- TypeScript API unchanged - only binary format changed
